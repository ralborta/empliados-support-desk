...
export type CleanLabApplicationConfig = Readonly<{
  ...
  openAiKey: string | null; openAiModel: string | null; waraBaseUrl: string | null; waraMaintenanceBaseUrl: string | null; waraToken: string | null;
  whatsappApiKey: string | null; whatsappTenantId: string | null;
...
export function loadCleanLabApplicationConfig(env: NodeJS.ProcessEnv = process.env): CleanLabApplicationConfig {
  ...
  const config: CleanLabApplicationConfig = Object.freeze({
    ...
    waraBaseUrl: optionalText(env, "WARA_API_BASE_URL"), waraMaintenanceBaseUrl: optionalText(env, "WARA_MAINTENANCE_API_BASE_URL") ?? optionalText(env, "WARA_API_BASE_URL"), waraToken: optionalText(env, "WARA_OBTENER_EMPRESA_TOKEN"),
    whatsappApiKey: optionalText(env, "WARA_CLEAN_WHATSAPP_API_KEY"), whatsappTenantId: optionalText(env, "WARA_CLEAN_WHATSAPP_TENANT_ID"),
  ...
  if (Boolean(config.whatsappApiKey) !== Boolean(config.whatsappTenantId)) throw new Error("INVALID_CLEAN_LAB_CONFIG:WHATSAPP:incomplete");
  if (config.whatsappTenantId && !config.allowedTenants.has(config.whatsappTenantId)) throw new Error("INVALID_CLEAN_LAB_CONFIG:WHATSAPP_TENANT:not_allowed");
