...
const pending = { operationId: "op", capability: "odometer.update", taskId: "task", version: 2, payloadHash: "hash", preparedArguments: { value: 100 }, status: "awaiting_confirmation" as const };
...
test("operation.prepare validates and creates a bound pending operation", () => {
  const result = operationPrepare({ operationId: "op", capability: "odometer.update", taskId: "task", version: 2, payloadHash: "hash", arguments: { value: 100 } });
  assert.equal(result.status, "resolved");
  if (result.status === "resolved") assert.deepEqual(result.value, pending);
  assert.equal(operationPrepare({ operationId: "", capability: "", taskId: "", version: 0, payloadHash: "", arguments: {} }).status, "invalid");
});
...
test("operation.confirm requires complete binding match", () => {
  assert.equal(operationConfirm({ pending, binding: { operationId: "op", capability: "odometer.update", taskId: "task", version: 2, payloadHash: "hash" } }).status, "resolved");
  assert.equal(operationConfirm({ pending, binding: { operationId: "other", capability: "odometer.update", taskId: "task", version: 2, payloadHash: "hash" } }).status, "invalid");
});
