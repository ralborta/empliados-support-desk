...
export interface Controller { decide(input: { interpretation: TurnInterpretation; state: ConversationStateClean; messageId: string }): TurnDecision; }
...
export type ConversationSaveResult = Readonly<{ status: "committed" | "duplicate"; state: ConversationStateClean; reply: string; traceId: string | null }>;
export interface ConversationStore { findReplay(input: { tenantId: string; conversationId: string; messageId: string }): Promise<ConversationSaveResult | null>; save(state: ConversationStateClean, context: ConversationSaveContext): Promise<ConversationSaveResult>; }
