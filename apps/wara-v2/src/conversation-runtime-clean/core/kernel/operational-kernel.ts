...
export type OperationBinding = Readonly<{ operationId: string; capability: string; taskId: string; version: number; payloadHash: string }>;
...
export function operationPrepare(input: PreparedOperationInput): KernelResult<PendingOperationState> {
  if (!input.operationId || !input.capability || !input.taskId || input.version <= 0 || !input.payloadHash) return { status: "invalid", errors: ["incomplete_operation_binding"] };
  return { status: "resolved", value: { operationId: input.operationId, capability: input.capability, taskId: input.taskId, version: input.version,
    payloadHash: input.payloadHash, preparedArguments: input.arguments, status: "awaiting_confirmation" }, facts: [] };
}
...
export function operationConfirm(input: { pending: PendingOperationState; binding: OperationBinding }): KernelResult<PendingOperationState> {
  ...
  if (pending.operationId !== binding.operationId || pending.capability !== binding.capability || pending.taskId !== binding.taskId
    || pending.version !== binding.version || pending.payloadHash !== binding.payloadHash) return { status: "invalid", errors: ["confirmation_binding_mismatch"] };
