...
export class AtomicCleanConversationStore implements ConversationStore {
  ...
  async findReplay(input: { tenantId: string; conversationId: string; messageId: string }) {
    const result = await this.repository.findReplay(input);
    return result ? { status: "duplicate" as const, state: result.record.state, reply: result.replayResult.reply, traceId: result.replayResult.traceId } : null;
  }
  async save(state: ConversationStateClean, context: ConversationSaveContext) {
    ...
    const result = await this.repository.commitTurn({ tenantId: state.tenantId, conversationId: state.conversationId, expectedVersion: this.expectedVersion, messageId: this.messageId, replayResult: { reply, traceId }, nextState: state, outbox, trace: { traceId, tenantId: state.tenantId, conversationId: state.conversationId, messageId: this.messageId, turnSequence: this.expectedVersion + 1, runtimeVersion: "clean-1", createdAt: now } });
    return { status: result.status, state: result.record.state, reply: result.replayResult.reply, traceId: result.replayResult.traceId } as const;
  }
}
