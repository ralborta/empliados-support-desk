...
import { CleanOptimisticConflictError, type CleanAtomicTurnCommit, type CleanCommitResult, type CleanDedupeRecord, type CleanPersistenceRepository, type CleanPersistenceSnapshot, type CleanStateRecord, type Clock } from "../../core/persistence/contracts.js";
...
export class InMemoryCleanPersistence implements CleanPersistenceRepository {
  ...
  async commitTurn(input: CleanAtomicTurnCommit): Promise<CleanCommitResult> {
    ...
    const duplicate = current?.dedupe.get(input.messageId);
    if (duplicate) return { status: "duplicate", record: structuredClone(currentRecord) };
    ...
    next.dedupe.set(input.messageId, { tenantId: input.tenantId, messageId: input.messageId, conversationId: input.conversationId, turnSequence: nextRecord.turnSequence, createdAt: now });
    if (input.outbox) {
    ...
    return { status: "committed", record: structuredClone(nextRecord) };
  }
