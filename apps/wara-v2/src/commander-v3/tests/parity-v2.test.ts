...
describe("commander-v3 parity V2 (KB + fechas + derivación)", () => {
  it("prompt version bump 16j", () => {
    assert.match(COMMANDER_V3_PROMPT_VERSION, /2026-08-16j/);
  });
  ...
  it("prompt Commander no copia el árbol de decisión V1", async () => {
    ...
    assert.match(COMMANDER_V3_SYSTEM_PROMPT, /lastQuestion\.expected es un SLOT|answerKind=status no es GPS/i);
  ...
  it("tras despedida + otra consulta → menú abierto (no 'sin información')", async () => {
    const { enrichPlanForOpenConsult } = await import("../enrich/open-consult.js");
    ...
    s.unit = {
      ...
      plate: "AG 396 ZA",
      name: "AG 396 ZA",
      label: "AG 396 ZA (M300-099)",
    };
    s.recentTurns = [
      {
        role: "assistant",
        text: "👍 Dale, cualquier cosa avisame.",
        at: new Date().toISOString(),
      },
    ];
    const badPlan = TurnPlanSchema.parse({
      reasoning: "inventó falta de info",
      conversationalAct: "inform",
      task: null,
      ...
      responseGoal: {
        purpose: "clarify",
        facts: [
          "No tengo información disponible en este momento.",
        ],
        nextQuestion:
          "¿Hay algo específico que necesites saber sobre El Cacique S.A.?",
      },
      confidence: 0.5,
    });
    const open = enrichPlanForOpenConsult(badPlan, s, "Hacer otra consulta");
    ...
    const greetish = TurnPlanSchema.parse({
      reasoning: "eligió greet",
  ...
  it("greet template solo si el turno es saludo; how_to no reenvía el menú", async () => {
    const { shouldUseGreetingTemplate } = await import("../reply/redact.js");
    const greet = TurnPlanSchema.parse({
      interpretation: { userQuestion: "saludo", answerKind: "greet" },
      reasoning: "saludo puro",
      conversationalAct: "greet",
      requestedCapabilities: [],
      stateIntent: { preserveCompany: true, preserveUnit: true, preserveTask: true },
      responseGoal: { purpose: "inform", facts: [] },
    ...
    assert.equal(shouldUseGreetingTemplate(greet), true);

    const howTo = TurnPlanSchema.parse({
      interpretation: {
    ...
    assert.equal(shouldUseGreetingTemplate(howTo), false);
  });
