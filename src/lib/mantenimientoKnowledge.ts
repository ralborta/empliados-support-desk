...
export const MANTENIMIENTO_ARTICLES: MantenimientoKnowledgeArticle[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "IMPORTANTE: el checkbox existe en pantalla, pero su semántica exacta NO está validada en el relevamiento (§11.5). Si preguntan qué significa: decí que el manual no lo confirma aún; no inventes definición.",
    ...
    restrictions: [
      "Semántica exacta de “Contar a partir de la realización”: pendiente §11.5 — no afirmar ni inventar definición.",
    ...
    relatedIds: ["mt-asignar-plan-unidad", "mt-concepto-y-mapa", "mt-contar-realizacion"],
    status: "available",
  },
  {
    id: "mt-contar-realizacion",
    category: "preventivo",
    title: "Contar a partir de la realización (pendiente de validación)",
    summary:
      "Checkbox del plan preventivo. Existe en UI; significado exacto NO confirmado (§11.5).",
    body: [
      "En Utilidades → Mantenimiento → Plan de mantenimiento → tarea preventiva aparece el checkbox “Contar a partir de la realización”.",
      "No se muestra cuando el criterio es “Cada N de <tarea>”.",
      "Estado del relevamiento: PENDIENTE DE VALIDACIÓN (§11.5). No hay definición verificada de qué cuenta ni desde cuándo.",
      "Respuesta correcta al cliente: el campo existe en el alta de la tarea preventiva, pero Atilio no puede afirmar qué hace exactamente hasta validarlo; sugerí probarlo con un administrador o soporte WARA si lo necesita operativo.",
      "NO confundir con “Confirmar la realización” del panel de tareas (otra acción, en Paneles).",
      "NO es un término de Transporte Público / hoja de turno.",
    ].join("\n"),
    source: { ...MANTENIMIENTO_SOURCE, pages: "2–3, §11.5" },
    restrictions: [
      "No inventar semántica de “Contar a partir de la realización”.",
    ],
    relatedIds: ["mt-plan-preventivo", "mt-panel-tareas"],
    status: "available",
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      "Después: (tilde) Confirmar la realización de la tarea (acción visible en el panel).",
      "Efecto exacto de esa confirmación sobre cierre de la tarea y recálculo del próximo vencimiento: PENDIENTE DE VALIDACIÓN — no afirmar como comprobado.",
      "Sin administración previa, la confirmación de realización es rechazada: “Sistema: No tiene realizada la administración del costo y artículos” (verificado).",
    ].join("\n"),
    source: { ...MANTENIMIENTO_SOURCE, pages: "5" },
    restrictions: [
      "Cierre de tarea + recálculo de próximo vencimiento tras “Confirmar la realización”: no end-to-end verificado — no afirmar.",
    ],
  ...
  },
  {
    ...
    body: [
      ...
      "6) Confirmar realización (tilde del panel). Efecto exacto sobre próximo vencimiento y estados de OT (p. ej. FINALIZADA): pendiente §11.3/§11.4 — no afirmar.",
      "7) Seguimiento en Informes → Órdenes de trabajo.",
    ].join("\n"),
    source: { ...MANTENIMIENTO_SOURCE, pages: "8" },
    restrictions: [
      "Correspondencia de estados OT y cierre por confirmación de realización: pendiente §11.3/§11.4.",
      "Alta OT punta a punta no ejecutada en el relevamiento (§11.4).",
    ],
