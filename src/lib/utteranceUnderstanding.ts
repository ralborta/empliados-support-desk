...
- Si el hilo YA tiene una unidad activa/confirmada (el bot dijo "Con la unidad X…" o "contame qué problema") y el cliente pide estado/reporte/GPS sin repetir la patente ("Quiero el estado", "el reporte", "¿cómo está?", "cómo ves", "indícame vos") → vehicle_unit, action=unit_status_read, unit_ref.none, clarify_question null. NUNCA pidas la matrícula de nuevo: usá la del hilo.
- Pedir ver/indicar el estado de una unidad ya referida ("cómo ves la …", "indícame", "mostrame cómo está") → action=unit_status_read (NO unit_reference). Tras un menú de síntomas, si el cliente insiste en ver el estado → unit_status_read otra vez (telemetría), no reiterar síntomas.
- Referencia contextual a la unidad ya activa sin repetir patente (el cliente no trae chapa nueva; unit_ref.kind=none) + pedido de estado → unit_status_read. Si solo confirma la unidad del hilo sin pedir estado → unit_reference + unit_ref.none.
